...
const captureWidget = async function capture(page, symbol, fileName, wait) {    
  ...
  await page.exposeFunction('message', onMessage);

  // Listening for click events on the window
  await page.evaluateOnNewDocument(() => {
      window.addEventListener('message', (event) => {
          window.message(JSON.parse(event.data))
      });
  });
...
const captureFull  = async function capture(page, symbol, fileName, wait) {    
  // Navigate the page to a URL
  const chartURL = `https://s.tradingview.com/widgetembed/?i=${symbol}&locale=br#{"symbol":"${symbol}","interval":"D","hide_top_toolbar":"1","allow_symbol_change":"1","save_image":"0","studies":"[]","theme":"dark","style":"1","timezone":"Etc/UTC","studies_overrides":"{}","utm_medium":"widget_new","utm_campaign":"chart","utm_term":"${symbol}","page-uri":"__NHTTP__"}`
...
for (let i=0; i<data.length; i++) {
  let asset = data[i]
  await captureFull(page, `${asset.exchange}:${asset.ticker}`, `x_${asset.exchange}-${asset.ticker}.jpg`, i==0);
}
...
// await capture(page, 'https://www.tradingview.com/chart/VZwoOSuF/?symbol=BLACKBULL:USDCAD&interval=D', '3.jpg');
...
// experiment
// https://www.tradingview.com/chart/VZwoOSuF/?symbol=BLACKBULL:USDCAD&interval=D#{"symbol":"BMFBOVESPA:SMTO3","interval":"D","hide_top_toolbar":"1","allow_symbol_change":"1","save_image":"0","watchlist":["BMFBOVESPA:SLCE3","BLACKBULL:USDCAD"],"studies":"[]","theme":"dark","style":"1","timezone":"Etc/UTC","studies_overrides":"{}","utm_medium":"widget_new","utm_campaign":"chart","utm_term":"BMFBOVESPA:SMTO3","page-uri":"__NHTTP__"}